...
│       ├── low-token-disabled       # Manual full-session LTM disable (/ltm session)
│       └── low-token-override-once  # Skip hook for one prompt (/ltm once, auto-deleted)
...
│   └── ltm/SKILL.md                 # /ltm — toggle low token mode (once|session|on|status)
